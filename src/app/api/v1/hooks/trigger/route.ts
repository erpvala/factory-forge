...
export async function POST(request: NextRequest) {
  try {
    ...
    if (!hasAuthToken || appSignature !== expectedSignature) {
      return NextResponse.json({ success: false, error: 'hook_trigger_unauthorized' }, { status: 403 });
    }

    const body = await request.json();
    const result = await triggerGlobalHooks({
    ...
    return NextResponse.json(
      { success: false, error: error instanceof Error ? error.message : 'hook_trigger_failed' },
      { status: 400 },
    );
  }
}
