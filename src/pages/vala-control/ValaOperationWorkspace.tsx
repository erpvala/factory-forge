...
export default function ValaOperationWorkspace() {
  ...
  useEffect(() => {
    if (user?.id) {
      ...
      supabase.from('audit_logs').insert({
        ...
        module: 'vala_operation',
        meta_json: { vala_id: id, role: 'operation' }
      });
    }
  }, [user?.id]);
  ...
  return (
    <ValaSecureWorkspace
      roleTitle="FRONT / OPERATION"
      valaId={valaId}
      onSessionExpire={handleSessionExpire}
    >
      <ValaIsolatedModule roleLevel="operation" valaId={valaId} />
    </ValaSecureWorkspace>
  );
}
